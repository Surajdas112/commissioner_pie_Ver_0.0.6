...
$getZone_COde = $_GET['ZCDR_CODE'];



$query = "SELECT zone.ZCDR_CODE,zone.ZCDR_NAME, p.num_gstn, p.not_pending, p.pending FROM (SELECT COMM_CODE,COUNT(GSTIN) as num_gstn, SUM(Flag=1) as not_pending, sum(Flag=0) as pending FROM nested_category WHERE ZONE_CODE=".$getZone_COde." GROUP by COMM_CODE) as p inner JOIN zone ON p.COMM_CODE = zone.ZCDR_CODE GROUP by 1";
//$query = "SELECT zone.COMM_CODE,zone.COMM_NAME, p.num_gstn, p.not_pending, p.pending FROM (SELECT COMM_CODE,COUNT(GSTIN) as num_gstn, SUM(Flag=1) as not_pending, sum(Flag=0) as pending FROM nested_category WHERE ZONE_CODE=".$getZone_COde." GROUP by COMM_CODE) as p inner JOIN zone ON p.COMM_CODE = zone.COMM_CODE GROUP by 1";
...
<body>
    ...
    <h1 style="text-align:center; border-bottom:1px solid red;">Directorate General of Analytics and Risk Management</h1>
        ...
        <h4 style="text-align:center; font-style:bold;margin-top:20px;">Commissionerate Wise Distribution</h4>
